...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    required init?(coder: NSCoder) {
        do {
            ...
            let newInventory = try InventoryUnarchiver.vendingInventory(fromDictionary: dictionary)
            self.vendingMachine = FoodVendingMachine(inventory: newInventory)
        } catch let error {
            fatalError("\(error)")
        }
    ...
    @IBAction func purchase() {
        if let currentSelection = currentSelection {
            ...
            // Reset the cell selection state.
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let currentSelection = currentSelection,
            let item = vendingMachine.item(forSelection: currentSelection) {
            let totalValue = item.price * Double(quantityStepper.value)
            updateDisplayWith(totalPrice: totalValue, itemPrice: item.price)
        }
    }
